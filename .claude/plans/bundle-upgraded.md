...
### Step 1: Atomic Bundle Migration ✅ COMPLETED

**Status**: ✅ COMPLETED

**Objective**: Remove deprecated `ActorBundle` struct and replace with Bevy 0.17+ component tuple pattern in `spawn_actor` function.

**Type**: ATOMIC - All changes must happen together (removing bundle breaks spawn_actor; new spawn_actor depends on standalone apply_rotations)

**Files Modified**:
- `src/actor/actor_spawner.rs`

**Changes**:
1. **DELETE**: `ActorBundle` struct definition and its `impl` block (lines 274-363)
   - Remove the `#[derive(Bundle)]` struct with 18 component fields
   - Remove the `ActorBundle::new()` constructor method

2. **CONVERT**: `apply_rotations` from method to standalone function (lines 367-385)
   - Keep the function logic exactly the same
   - Remove it from `impl ActorBundle` context
   - Place it as a standalone module-level function

3. **REPLACE**: `spawn_actor` function implementation (line 499)
   - Extract parent components (transform, velocity, aabb)
   - Calculate spawn transform using `config.calculate_spawn_transform()`
   - Apply rotation logic using standalone `apply_rotations()` helper
   - Calculate velocities using `config.velocity_behavior.calculate_velocity()`
   - Spawn entity with component tuple containing all 20 components
...
- ✅ No deprecation warnings about bundles
- ✅ All spawn call sites (spaceship, missile, nateroid) remain unchanged
- ✅ Exact same runtime behavior as before

**Implementation Details**: See "Phase 1 & Phase 2 Implementation" section below for complete before/after code.

**Notes**:
- This preserves the existing two-phase rotation application pattern (calculate_spawn_transform applies rotation, then apply_rotations may overwrite when combining with parent)
- The intermediate rotation application is redundant but functionally correct - this is existing behavior being preserved
- No call sites need updating (spaceship.rs, missile.rs, nateroid.rs stay unchanged)

---

### Step 2: Validation and Quality Checks ⏳ PENDING

**Status**: ⏳ PENDING

**Objective**: Run automated checks and manual testing to verify migration success.
...
- ✅ Game runs identically to before migration
...
**ActorBundle struct** (lines 274-363):
```rust
#[derive(Bundle)]
...
**spawn_actor function** (replaces line 499):
```rust
pub fn spawn_actor<'a>(
    commands: &'a mut Commands,
    config: &ActorConfig,
...
- Convert: `apply_rotations` from method to standalone function
...
- **Added**: ~20 lines (inline component tuple + standalone function)
- **Net change**: ~-70 lines
