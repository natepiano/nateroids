...
CAMERA=$(curl -s -X POST http://127.0.0.1:$PORT/jsonrpc -H "Content-Type: application/json" \
  -d '{"jsonrpc": "2.0", "id": 1, "method": "world.query", "params": {"filter": {"with": ["hana_lagrange::orbit_cam::OrbitCam"]}, "data": {}}}' \
...
while true; do
  ...
  curl -s -X POST http://127.0.0.1:$PORT/jsonrpc -H "Content-Type: application/json" \
    -d "{\"jsonrpc\": \"2.0\", \"id\": 1, \"method\": \"world.trigger_event\", \"params\": {\"event\": \"hana_lagrange::fit::triggers::zoom::ZoomToFit\", \"value\": {\"camera\": $CAMERA, \"target\": $target, \"margin\": 0.1, \"anchor\": \"Center\", \"offset_px\": [0.0, 0.0], \"duration\": {\"secs\": 0, \"nanos\": 500000000}, \"easing\": \"CubicOut\"}}}" >/dev/null 2>&1
