...
while true; do
  ...
  curl -s -X POST http://127.0.0.1:$PORT/jsonrpc -H "Content-Type: application/json" \
    -d "{\"jsonrpc\": \"2.0\", \"id\": 1, \"method\": \"world.trigger_event\", \"params\": {\"event\": \"bevy_panorbit_camera_ext::extension::ZoomToFitMesh\", \"value\": {\"entity\": $CAMERA, \"target_entity\": $target}}}" >/dev/null 2>&1
